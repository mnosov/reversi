...
public:
  ...
  uint score(Defs::ChipColor color) const     { return m_score[color]; }

  void set(Defs::ChipColor color, uint score) { m_score[color] = score; }
  void inc(Defs::ChipColor color)             { m_score[color]++; }
  void dec(Defs::ChipColor color)             { m_score[color]--; }
  void add(Defs::ChipColor color, uint s)     { m_score[color] += s; }
  void sub(Defs::ChipColor color, uint s)     { m_score[color] -= s; }
...
KReversiPos Engine::computeMove(const GameInterface& game, bool competitive)
{
    if( m_computingMove )
    {
        qDebug() << "I'm already computing move! Yours KReversi Engine.";
        return KReversiPos();
    }
  ...
  m_score->set(Defs::White, game.playerScore(Defs::White));
  m_score->set(Defs::Black, game.playerScore(Defs::Black));
  ...
  m_depth = m_strength;
  if (m_score->score(Defs::White) + m_score->score(Defs::Black) + m_depth + 3 >= 64)
    m_depth = 64 - m_score->score(Defs::White) - m_score->score(Defs::Black);
  else if (m_score->score(Defs::White) + m_score->score(Defs::Black) + m_depth + 4 >= 64)
    m_depth += 2;
  else if (m_score->score(Defs::White) + m_score->score(Defs::Black) + m_depth + 5 >= 64)
    m_depth++;

  // If we are very close to the end, we can even make the search
  // exhaustive.
  if (m_score->score(Defs::White) + m_score->score(Defs::Black) + m_depth >= 64)
    m_exhaustive = true;
  ...
  // pieces.
  m_coeff = 100 - (100*
                   (m_score->score(Defs::White) + m_score->score(Defs::Black)
		    + m_depth - 4)) / 60;
  ...
  m_bc_score->set(Defs::White, CalcBcScore(Defs::White));
  m_bc_score->set(Defs::Black, CalcBcScore(Defs::Black));
  ...
  for (int x = 1; x < 9; x++) {
	...
	if (val > maxval) {
	  ...
	  // dialog.
          int randi = qrand() % 7; //m_random.getLong(7);
	  if (maxval == -LARGEINT
	      || m_competitive
...
//      qDebug() << "Move " << k << ":" << moves[k].m_value << " (" << moves[k].m_x << ", " << moves[k].m_y << ")";
//  }
  if (number_of_maxval > 1) {
    int  r = (qrand() % number_of_maxval) + 1;
    int  i;
  ...
  if (interrupted())
    return KReversiPos(Defs::NoColor, -1, -1);
...
KReversiPos Engine::ComputeFirstMove(const GameInterface& game)
{
  int    r;
  Defs::ChipColor  color = game.currentPlayer();
...
int Engine::ComputeMove2(int xplay, int yplay, Defs::ChipColor color, int level,
			 int cutoffval, quint64 colorbits,
			 quint64 opponentbits)
{
  int               number_of_turned = 0;
  SquareStackEntry  mse;
  Defs::ChipColor             opponent = opponentColorFor(color);
  ...
  m_board[xplay][yplay] = Defs::NoColor;
  m_score->sub(color, 1);
...
int Engine::TryAllMoves(Defs::ChipColor opponent, int level, int cutoffval,
...
int Engine::EvaluatePosition(Defs::ChipColor color)
{
  int retval;
...
int Engine::CalcBcScore(Defs::ChipColor color)
{
  int sum = 0;
...
quint64 Engine::ComputeOccupiedBits(Defs::ChipColor color)
{
  quint64 retval = 0;
