...
static Defs::ChipColor opponentColorFor(Defs::ChipColor color)
{
...
public:
     ...
     */
    virtual Defs::ChipColor currentPlayer() const = 0;

    /**
     *  @return score (number of chips) of the player
     */
    virtual int playerScore( Defs::ChipColor player ) const = 0;
    /**
     *  @return color of the chip at position [row, col]
     */
    virtual Defs::ChipColor chipColorAt( int row, int col ) const = 0;

    /**
     *  @return if move is possible to specified position by specified color
     */
    virtual bool isMovePossible( const KReversiPos& move ) const = 0;
...
public:
  ...
  KReversiPos     computeMove(const GameInterface& game, bool competitive);
...
private:
  KReversiPos     ComputeFirstMove(const GameInterface& game);
  int      ComputeMove2(int xplay, int yplay, Defs::ChipColor color, int level,
  int      cutoffval,
  quint64  colorbits, quint64 opponentbits);

  int      TryAllMoves(Defs::ChipColor opponent, int level, int cutoffval,
  quint64  opponentbits, quint64 colorbits);

  int      EvaluatePosition(Defs::ChipColor color);
  void     SetupBcBoard();
  void     SetupBits();
  int      CalcBcScore(Defs::ChipColor color);
  quint64  ComputeOccupiedBits(Defs::ChipColor color);

  void yield();

private:

  Defs::ChipColor        m_board[10][10];
  int          m_bc_board[9][9];
  Score*        m_score;
  Score*        m_bc_score;
